<template>
    <div class="text-center">
        <b-card no-body class="overflow-hidden tarjeta" v-for="(item, index) in usuarios" :key="item.id.value" border-variant="success">
            <b-row no-gutters>
                <b-col md="4">
                    <b-card-img :src="usuarios[index].picture.large" :alt="usuarios[index].name.first" :title="usuarios[index].name.first" img-left class="imagen shadow p-3 mb-2 bg-white rounded"></b-card-img>
                </b-col>
                <b-col md="8">
                    <b-card-body class="cuerpo-tarjeta">
                        <b-card-text>
                            <strong>Nombre:</strong> {{usuarios[index].name.first + ' ' + usuarios[index].name.last}} <br>
                            <strong>Correo:</strong> {{usuarios[index].email}}<br>
                            <strong>Pais:</strong> {{usuarios[index].location.country}}
                        </b-card-text>
                        <b-row>
                            <b-col>
